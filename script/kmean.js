...
function getIndexFromCentroids(centroid, centroids) {
  for(let i = 0; i < centroids.length; i++) {
    const data = centroids[i];
    if(centroid[0]== data[0] &&  centroid[1]== data[1]) {
      return i;
    }
  }
  return -1;
...
function kmeans(dataset, k, useNaiveSharding = true) {
  if (dataset.length && dataset[0].length && dataset.length > k) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      const {points, centroid} = labels[i];
      for(let m = 0; m < points.length; m++) {
        indexData.push(getIndexFromCentroids(centroid, centroids));
      }
    }
    for(let i = 0; i < centroids.length; i++) {
